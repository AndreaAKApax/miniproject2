...
public class AVLTree<E extends Comparable<E>> {
    ...
    public E getSuccessor(E el) {
    	...
    	//controllo se l'elemento è presente nell'alber
    	AVLTreeNode input = this.root.search(el);    	
    	if (input == null)
    		throw new IllegalArgumentException("L'elemento non è presente nell'albero");
    	
    	//controllo prima se è l'elemento massimo, sennò cerco il successore
    	if (!this.getMaximum().equals(el)) {
    ...
    public E getPredecessor(E el) {
    	...
    	//controllo se l'elemento è presente nell'albero
    	AVLTreeNode input = this.root.search(el);    	
    	if (input == null)
    		throw new IllegalArgumentException("L'elemento non è presente nell'albero");
    	
    	//controllo prima se e l'elemento minimo, sennò cerco il predecessore
    	if (!this.getMinimum().equals(el)) {
    ...
    public class AVLTreeNode {
        ...
        public int insert(E el) {
            ...
            if (!this.isBalanced()) {
            	...
            	//int leftSonBF = this.left.getBalanceFactor();
            	//int rightSonBF = this.right.getBalanceFactor();
        ...
        private void rotazioneSS (AVLTreeNode actualRoot, AVLTreeNode nextRoot) {
        	//il padre di actualRoot diventa quello della nuova radice
        	nextRoot.parent = actualRoot.getParent();
        	//sfruttando la proprietà dei BST verifico se actualRoot era un figlio dx o sx
        	int posizione = actualRoot.getParent().getEl().compareTo(actualRoot.el);        	
        	if (posizione > 0)
        		//actualRoot era il figlio sx
        		actualRoot.getParent().setLeft(nextRoot);
        	else
        		//actualRoot era il filgio dx
        		actualRoot.getParent().setRight(nextRoot);
        	
        	//asseggno il filgio dx di nextRoot come figlio sx di actualRoot
        	AVLTreeNode subtree = nextRoot.right;  
            actualRoot.left = subtree;
            if (subtree != null)
                subtree.parent = actualRoot;
            
            //concludo la rotazione
        ...
        private void rotazioneSD (AVLTreeNode actualRoot, AVLTreeNode actualRootLeftSon) {
        	//il padre di actualRoot diventa quello della nuova radice
        	actualRootLeftSon.right.parent = actualRoot.getParent();
        	//solo se questo nodo padre esiste
        	//sfruttando la proprietà dei BST verifico se actualRoot era un figlio dx o sx
        	if (actualRoot.getParent() != null) {
        		int posizione = actualRoot.getParent().getEl().compareTo(actualRoot.el);        	
        		if (posizione > 0)
        			//actualRoot era il figlio sx
        			actualRoot.getParent().setLeft(actualRootLeftSon.right);
        		else
        			//actualRoot era il filgio dx
        			actualRoot.getParent().setRight(actualRootLeftSon.right);}
        	...
        	//aggiorno le altezze
        	actualRoot.updateHeight();
        	actualRootLeftSon.updateHeight();
            nextRoot.updateHeight();
        }
        ...
        private void rotazioneDD (AVLTreeNode actualRoot, AVLTreeNode nextRoot) {               	           
        	...
        	//asseggno il filgio sx di nextRoot come figlio dx di actualRoot
        ...
        private void rotazioneDS (AVLTreeNode actualRoot, AVLTreeNode actualRootRightSon) {
        	//il padre di actualRoot diventa quello della nuova radice
        	actualRootRightSon.left.parent = actualRoot.getParent();
        	//sfruttando la proprietà dei BST verifico se actualRoot era un figlio dx o sx
        	int posizione = actualRoot.getParent().getEl().compareTo(actualRoot.el);        	
        	if (posizione > 0)
        		//actualRoot era il figlio sx
        		actualRoot.getParent().setLeft(actualRootRightSon.left);
        	else
        		//actualRoot era il filgio dx
        		actualRoot.getParent().setRight(actualRootRightSon.left);
        	...
        	//se questo nodo esiste gli assegno come parent actualRoot
        	actualRoot.right = nextRootLeftSon;
        	if(nextRootLeftSon != null)
        	...
        	//aggiorno le altezze
        	actualRoot.updateHeight();
        	actualRootRightSon.updateHeight();
            nextRoot.updateHeight();
        }
